...
import {
  ...
  definePost,
  definePut,
} from '../schema/define-http-endpoint'
import { defineSSE } from '../schema/stream-response'
...
describe('create-api-client', () => {
  describe('Rest endpoints', () => {
    ...
    describe('Advanced API scenarios', () => {
      ...
      it('should handle complex query parameter scenarios', async () => {
        ...
        expect(url.searchParams.getAll('filters')).toEqual([
          'filter1',
          'filter2',
        ])
  ...
  //     const apiSchema = {
  //       notificationStream: defineSSE('/events/notifications', {
  //         input: {
  //           query: {
  //             userId: z.string(),
  //           },
  ...
  //       zValidator(
  //         'query',
  //         z.object(apiSchema.notificationStream.input?.query),
  //       ),
  //       (c) =>
  ...
  //     const promise = client.events.notifications.$sse({
  //       query: { userId: '123' },
  //       onMessage: (data) => {
  ...
  //     const apiSchema = {
  //       userEvents: defineSSE('/users/:userId/events', {
  //         input: {
  //           query: {
  ...
  //       const url = new URL(request.url)
  //       expect(url.pathname).toContain('/users/user123/events')
  //       expect(url.searchParams.get('since')).toBe('2023-01-01')

  //       const stream = new ReadableStream({
  //         start(controller) {
  //           controller.enqueue(
  //             new TextEncoder().encode(
  //               'data: {"userId":"user123","field":"name","newValue":"John Smith"}\n\n',
  //             ),
  //           )
  //           controller.enqueue(
  //             new TextEncoder().encode(
  //               'data: {"userId":"user123","action":"login","timestamp":"2023-01-01T12:00:00Z"}\n\n',
  //             ),
  //           )
  //           controller.close()
  //         },
  ...
  //     await client.users.userId('user123').events.$sse({
  ...
  //           // Send valid event
  //           controller.enqueue(
  //             new TextEncoder().encode('data: {"message":"valid"}\n\n'),
  //           )
  //           controller.close()
  //         },
  //       })
  ...
  //     // Should receive one valid message despite the malformed one
  //     expect(receivedMessages).toHaveLength(1)
  //     expect(receivedMessages[0]).toEqual({ message: 'valid' })

  //     // Should have one error for the malformed data
  //     expect(errors).toHaveLength(1)
  //     expect(errors[0]).toBeInstanceOf(ApiError)
  //   })
  ...
  //     const apiSchema = {
  //       secureEvents: defineSSE('/secure/events', {
  //         input: {
  //           query: {
  //             token: z.string(),
  //           },
  ...
  //     await client.secure.events.$sse({
  //       query: { token: 'abc123' },
  //       headers: {
  ...
  //     expect(receivedMessages).toHaveLength(1)
  //     expect(receivedMessages[0]).toEqual({ encrypted: 'encrypted-data' })
  //   })
  ...
  //           // In a real scenario, this would be interrupted by the abort signal
  //         },
  //       })
  ...
  //     const apiSchema = {
  //       multiEvents: defineSSE('/multi-events', {
  //         events: {
  //           typeA: z.object({
  //             type: z.literal('A'),
  //             data: z.string(),
  ...
  //           controller.enqueue(
  //             new TextEncoder().encode(
  //               'data: {"type":"A","data":"string data"}\n\n',
  //             ),
  //           )
  //           controller.enqueue(
  //             new TextEncoder().encode('data: {"type":"B","value":42}\n\n'),
  //           )
  //           controller.enqueue(
  //             new TextEncoder().encode(
  //               'data: {"type":"C","items":["item1","item2"]}\n\n',
  //             ),
  //           )
  //           controller.close()
  //         },
  ...
  //     const result = await client['multi-events'].$sse({
  //       onMessage: (data: any) => {
  //         receivedMessages.push(data)
  //       },
  //     })
  ...
  //     expect(receivedMessages[0]).toEqual({ type: 'A', data: 'string data' })
  //     expect(receivedMessages[1]).toEqual({ type: 'B', value: 42 })
  //     expect(receivedMessages[2]).toEqual({
  //       type: 'C',
  //       items: ['item1', 'item2'],
  //     })

  //     // Check that the result contains the accumulated response data
  //     expect(result).toEqual({
  //       typeA: { type: 'A', data: 'string data' },
  //       typeB: { type: 'B', value: 42 },
  //       typeC: { type: 'C', items: ['item1', 'item2'] },
  //     })
  //   })
  // })
})
